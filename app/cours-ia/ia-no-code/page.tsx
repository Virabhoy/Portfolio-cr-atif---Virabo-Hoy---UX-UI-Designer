...
import NoCodeVsDev from "@/components/course/ia-no-code/NoCodeVsDev";
import NoCodeToolsGrid from "@/components/course/ia-no-code/NoCodeToolsGrid";

export default function IaNoCodePage() {
  const { language } = useLanguage();
  const course = iaNoCodeData;
  const lang = language as "fr" | "en";

  const illustrationAfterSection: Record<string, React.ComponentType> = {
    "no-code-vs-dev": NoCodeVsDev,
    "what-is-ai-no-code": NoCodeToolsGrid,
  };
  ...
  return (
    <>
      ...
      <CourseNavFooter basePath="/cours-ia"
        nextChapter={{
          slug: iaAutomationData.slug,
          title: iaAutomationData.title[lang],
        }}
      />
    </>
  );
