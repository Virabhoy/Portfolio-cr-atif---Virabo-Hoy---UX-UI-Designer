...
import AiToolsEcosystem from "@/components/course/ia-outils-marche/AiToolsEcosystem";
import PricingComparison from "@/components/course/ia-outils-marche/PricingComparison";

const illustrationAfterSection: Record<string, React.ComponentType> = {
  "hook": AiToolsEcosystem,
  "pricing-comparison": PricingComparison,
};

export default function IaOutilsPage() {
  const { language } = useLanguage();
  const course = iaOutilsMarcheData;
  ...
  return (
    <>
      ...
      <CourseNavFooter basePath="/cours-ia"
        nextChapter={{
          slug: "ia-claude-code",
          title: iaClaudeCodeData.title[lang],
        }}
      />
    </>
  );
