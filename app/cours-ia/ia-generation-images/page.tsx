...
import ImageGenLandscape from "@/components/course/ia-generation-images/ImageGenLandscape";
import PromptToImage from "@/components/course/ia-generation-images/PromptToImage";

const illustrationAfterSection: Record<string, React.ComponentType> = {
  "landscape": ImageGenLandscape,
  "prompting": PromptToImage,
};

export default function IaImageGenPage() {
  const { language } = useLanguage();
  const course = iaImageGenerationData;
  ...
  return (
    <>
      ...
      <CourseNavFooter basePath="/cours-ia"
        nextChapter={{
          slug: iaCodingAssistantsData.slug,
          title: iaCodingAssistantsData.title[lang],
        }}
      />
    </>
  );
