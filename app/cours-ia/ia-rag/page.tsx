...
import RagPipeline from "@/components/course/ia-rag/RagPipeline";
import RagVsFinetuning from "@/components/course/ia-rag/RagVsFinetuning";

export default function IaRagPage() {
  const { language } = useLanguage();
  const course = iaRagData;
  const lang = language as "fr" | "en";

  const illustrationAfterSection: Record<string, React.ComponentType> = {
    "how-it-works": RagPipeline,
    "rag-vs-finetuning": RagVsFinetuning,
  };
  ...
  return (
    <>
      ...
      <CourseNavFooter basePath="/cours-ia"
        nextChapter={{
          slug: iaDataCultureData.slug,
          title: iaDataCultureData.title[lang],
        }}
      />
    </>
  );
